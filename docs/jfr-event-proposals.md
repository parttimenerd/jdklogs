...
**Last updated**: 2026-08-19 (pass 79)
...
| 2 | jdk.GCOverheadLimitExceeded | **Propose with caveats** | High | `log_info(gc)` (decision log); `memAllocator.cpp` (JFR emission site) | OOM throw: GC overhead limit exceeded | **Easy** — single throw point per collector; fields are accessor calls on the policy object at the emission site; ~30 lines per collector; caveat: G1 and Parallel have different free-space field shapes (one `freeSpacePercent` vs. two split fields) |
...
| 4 | jdk.ShenandoahCollectionDecision | **Propose with caveats** | High | `log_info(gc,ergo)` / `log_info(gc)` | Shenandoah GC cycle start | **Medium** — initial submission is base-fields-only from single `service_concurrent_normal_cycle()` site (~30 lines); trigger fields (`jdk.ShenandoahGCTrigger`) are a separate follow-up event |
...
| 11 | jdk.G1CollectionSetCandidates | **Propose with caveats** | Medium | `log_debug(gc,ergo,cset)` | Each mixed GC CSet finalization | **Medium** — `stopReason` uses existing string literals directly (no enum needed for initial submission); emit from Marking path only first; log-level promotion needed; ~50 lines |
...
**Propose with caveats.** Log level is `log_info(trimnative)` — correct tier. One prerequisite code change is required: the RSS data collection (`sc.before`/`sc.after` from `/proc/self/status`) is currently gated on `logging_enabled`, so it is not collected when `-Xlog:trimnative=info` is off. The fix is to pass the `SizingCollection*` unconditionally to `os::trim_native_heap()` (or gate on `logging_enabled || jfr_event_enabled`). This change must be bundled in the same PR. See OQ1 for the full analysis.
...
| `gcId` | `GCId::peek() - 1` — last assigned GC id; `peek()` returns `_next_id` (the NEXT id to be assigned), so `peek()-1` gives the last completed GC id. **Implementation safety note**: `peek()` returns 1 before any GC has completed (`_next_id` starts at 1), so `peek()-1 = 0 = GCId::undefined()`. The implementation must guard: `if (GCId::peek() > 1) gcId = GCId::peek() - 1; else gcId = GCId::undefined()`. | Yes | Yes | Yes (undefined if no GC has run yet) | Join with `jdk.GarbageCollection` on this `gcId` to retrieve the last GC's type (`cause`), duration, and whether it was a Full GC or a minor GC — a long Full GC immediately before the throw confirms the GC was doing maximum work; a short minor GC confirms old gen was not being touched. Also join with `jdk.G1HeapSummary` (G1) or `jdk.PSHeapSummary` (Parallel) on this `gcId` to see heap occupancy after the last GC — the used/committed ratio from those events gives the starting conditions for the OOM sequence. Null when no GC has completed; in that case the JVM threw OOM on the very first allocation attempt (extremely unusual — implies heap too small to start). |
...
**Propose with caveats.** Use the simplified 5-field version for the initial upstream submission (`startTime`, `freePercent`, `goodProgress`, `failedDimension`, `badProgressCount`). The 12-field full version with nullable dimensions will receive pushback. `badProgressCount` is the single most actionable field and must be present in any version. **Fallback if the full event is rejected**: propose `badProgressCount` as a single new field on `jdk.GarbageCollection` — this is a one-field extension to an existing event and is the minimal viable contribution.
...
**Propose with caveats.** Info-level source, single emission point, easy implementation. Production motivation is currently weak — the event should not be filed upstream until at least one of the following is available: (a) a benchmark or profiling trace showing nmethod scan overhead > 5% of ZGC collection time; (b) a JDK bug or mailing list thread reporting nmethod-table-related GC overhead; (c) a GraalVM or large-polyglot deployment case study with high `registeredNMethods`. Without concrete evidence, upstream will ask "why is this needed?" and the answer must be quantitative. The implementation is trivial; the bottleneck is justification, not code.
...
| `exceededGoal` | Boolean: `pendingCards > pendingCardsTarget` — pending backlog exceeds the policy's target. **Note: this field is fully derivable from `pendingCards > pendingCardsTarget` on the same event.** It is retained because it is the explicit branch variable in the `adjust_threads_wanted()` code, making JFR consumers' intent clear without requiring a comparison expression in every query; upstream reviewers may ask to drop it as redundant — the counter-argument is query ergonomics. | No | **Three-branch resolution**: (a) `exceededGoal=true` with `threadsWanted < G1ConcRefinementThreads` → policy wants more threads than available; increase `-XX:G1ConcRefinementThreads`; (b) `exceededGoal=true` with `threadsWanted` at max AND `goalMs` is small (check from same event) → the time budget for refinement is too tight; raise `-XX:G1RSetUpdatingPauseTimePercent` (each 1% increase raises `goalMs` proportionally; cap at 25%) or raise `-XX:MaxGCPauseMillis` to increase the absolute budget; (c) `exceededGoal=true` with `threadsWanted` at max AND `goalMs` is adequate → write rate fundamentally exceeds refinement throughput regardless of thread count or budget; only reducing cross-region write rate at the application level resolves this. |
...
2. **`stopReason` implementation**: the simplest approach for the initial submission is to pass the existing string literals from `print_finish_message()` directly as the `stopReason` field value — the four strings (`"Predicted time too high"`, `"Maximum number of regions reached"`, `"Region amount reached min"`, `"Candidates exhausted"`) already exist and are stable. A `StopReason` enum is cleaner C++ and is the recommended long-term form, but is not required to land the event. Upstream's concern about `stopReason` is whether the field is too implementation-specific — not the mechanics. The counter-argument: `stopReason` is exactly what the Oracle tuning guide's three mixed-GC knobs (`G1MixedGCCountTarget`, `G1MixedGCLiveThresholdPercent`, `G1HeapWastePercent`) are designed to address, and each maps to exactly one `stopReason` value. File with string literals; refactor to enum in a follow-up if upstream prefers.
...
2. **`stopReason` implementation — concrete options**: (a) **Simple, initial**: pass the existing string literals from `print_finish_message()` directly — no C++ changes needed, just read the string at the emit site. (b) **Preferred, follow-up**: replace with a `StopReason` local enum (`REGION_CAP_REACHED`, `MIN_REGIONS_MET`, `TIME_BUDGET_EXHAUSTED`, `CANDIDATES_EXHAUSTED`), resolve to string in the JFR event. Option (a) is acceptable for landing the event; option (b) is the cleaner long-term form. Either way the upstream concern is the field's existence, not the mechanics.
3. **Resolved: Marking path only for initial submission; Retained as follow-up.** The initial proposal emits from `select_candidates_from_marking()` only. This covers the most diagnostically important case — Marking selection is where the `"Predicted time too high"` and `"Maximum number of regions reached"` stop reasons occur, and where `availableRegions` vs. `selectedRegions` gives the most actionable signal. Adding Retained-path emission is a follow-up: `select_candidates_from_retained()` has different stop-reason semantics (`"Retained candidates exhausted"` is the normal exit) and the Retained pass runs less frequently (only when retained candidates exist). Separating into two PRs keeps the initial diff focused and reduces the surface area for upstream review.
...
3. **Emission gate**: must be gated on `resizeBytes != 0`. The `expand` flag in `young_collection_resize_amount()` is only set inside the expand/shrink branches — when neither fires, it retains its initial `false` value. The caller in `g1CollectedHeap.cpp` already guards on `resize_bytes != 0` before acting. The JFR event must apply the same gate to avoid emitting per-pause noise with stale field values.
...
**Propose with caveats.** The 22-field sparse original design was the blocker; that has been replaced by the event documented below. The initial submission uses only the 3 fields computable at `start_gc()` — `heapFreePercent`, `triggeredMinorRule`, `triggeredMajorRule` — requiring zero struct changes to `zDirector.cpp`. `timeUntilMinorOOM` and `minorFreeBytes`, which need a `ZDirectorResult` struct to propagate from an inner rule function, are explicitly deferred to a follow-up PR. Remaining caveats: (1) all source sites are `log_debug(gc,director)` — the upstream submission should include log-level promotion of the most relevant director log to `log_info` in the same PR; (2) the emission gate (OQ2) must suppress idle-tick noise. With these two addressed, the initial event is upstreamable. The `timeUntilMinorOOM` follow-up is documented in OQ3.
...
| `timeUntilMinorOOM` | **DEFERRED to follow-up PR** — requires `ZDirectorResult` struct propagation from `rule_minor_allocation_rate_dynamic()` (see OQ3). Not in the initial submission. Once landed: projected seconds until young-gen exhaustion at current allocation rate. > 10s = safe; 2–5s = pressure building; < 2s = stalls imminent. Null if the rate rule was not the firing rule. | Yes | See OQ3 for implementation path. |
| `minorFreeBytes` | **DEFERRED to follow-up PR** — same struct dependency as `timeUntilMinorOOM`. Young-gen remaining headroom at tick time; `timeUntilMinorOOM` = `minorFreeBytes / maxAllocRate`. | Yes | See OQ3 for implementation path. |
...
**Cross-event correlation**: join `jdk.ZDirectorRule` with `jdk.ZYoungGarbageCollection` or `jdk.ZOldGarbageCollection` by time proximity — every event where `triggeredMinorRule` or `triggeredMajorRule` is non-null should be followed within a few seconds by a corresponding GC event. If a triggered-rule event is NOT followed by a GC event, the director intended to start a GC but something blocked it (e.g., a concurrent GC was already running). Join with `jdk.ZAllocationStall` by time window: if `jdk.ZAllocationStall` events appear shortly after a triggered tick, the director's decision was correct but the GC did not start fast enough — this identifies cases where the 1-second tick interval is too coarse for the allocation rate. Track `heapFreePercent` across consecutive ticks to estimate the allocation rate independently of the follow-up `timeUntilMinorOOM` field.

#### Why existing events don't cover this

- `jdk.ZYoungGarbageCollection`: fires after a GC is chosen and completed; records `tenuringThreshold`, `pause` duration, and cause. Does not capture ticks where no GC triggered — the most important case for proactive diagnosis. No rule name, no heap-free fraction at decision time. `timeUntilMinorOOM` (deferred follow-up field) is the director's OOM prediction — not available in any existing event.
- `jdk.ZOldGarbageCollection`: same limitation — outcome event after an old collection completes. No director rule fields, no `heapFreePercent`, and no non-triggering ticks.
- `jdk.ZAllocationStall`: fires when an allocating thread had to stall waiting for memory — the **failure case** that ZGC's proactive director is supposed to prevent. `jdk.ZDirectorRule` is complementary: it shows the prevention-side decisions; `jdk.ZAllocationStall` shows when prevention failed.
...
`jdk.ZDirectorRule` directly exposes the director tick data that drives this proactive triggering — currently only visible via `-Xlog:gc+director=debug`. The `triggeredMinorRule` and `triggeredMajorRule` fields name the rule that fired; `heapFreePercent` captures the heap state at decision time. `timeUntilMinorOOM` (deferred to follow-up) is the director's OOM-prediction value — the core of ZGC's proactive model.
...
1. **All-debug source**: `zDirector.cpp` has zero `log_info` sites. This is the hardest upstream justification. The argument combines: (a) director tick data is production-relevant and non-triggering ticks produce zero JFR signal today; (b) JFR's access model is independent of `-Xlog` level — the code executes unconditionally in production builds; (c) the emission gate (OQ2) limits recording volume to trigger ticks and near-full-heap ticks only. Best approach for submission: propose log-level promotion of the triggered-rule log sites inside the individual rule functions (e.g., `rule_minor_high_usage`, `rule_minor_allocation_rate_dynamic`) from `log_debug(gc,director)` to `log_info(gc,director)` in the same PR — this addresses the root concern while adding JFR coverage simultaneously. Note: `start_gc()` itself does not log; only the rule functions log.
2. **Emission gate**: gate on `triggeredMinorRule != null || triggeredMajorRule != null || heapFreePercent < 10.0`. Emitting every second (~86,400/day) when idle generates recording overhead with no diagnostic value. For the initial 3-field submission the `heapFreePercent < 10.0` guard is the only threshold needed — it fires when occupancy passes 90%, providing warning before `rule_minor_high_usage` fires at 95%. The `timeUntilMinorOOM < 30.0` guard is deferred to the follow-up PR (it requires the struct — see OQ3).
3. **`timeUntilMinorOOM` and `minorFreeBytes` — follow-up PR**: these two fields are locals inside `rule_minor_allocation_rate_dynamic()` and do not flow to `start_gc()`. Adding them requires a small output struct:
...
`make_minor_gc_decision()` returns `ZDirectorResult` alongside `GCCause::Cause`. `rule_minor_allocation_rate_dynamic()` populates `time_until_oom` into the struct before returning. The change is ~15 lines across 3 functions. This is the correct long-term form and should be filed as a follow-up to the initial event landing — keeping it separate avoids blocking the simpler initial submission on struct API review.
...
The three phases have materially different cardinalities: Site 1 fires once per old-gen marking cycle completion (infrequent), Sites 2 and 3 fire once per mixed-evacuation cycle (more frequent), and Site 4 fires only when the threshold changes (rare). Upstream reviewers will likely ask why these are combined in one event given the cardinality difference. **Recommended approach for upstream submission**: file as three separate events — `jdk.ShenandoahOldGarbageSurvey` (Site 1), `jdk.ShenandoahOldEvacuationOutcome` (Sites 2 and 3), `jdk.ShenandoahOldThresholdAdjust` (Site 4). This avoids nullable fields, makes each event independently queryable, and matches the OpenJDK convention of one event per distinct log site concern. The fields below are grouped by their originating phase; in the three-event form each group becomes its own event with only non-null fields.
...
| `phase` | String discriminator: `"preparation"` (Site 1), `"evacuation_outcome"` (Site 2 — normal, all-pinned, or no-budget paths), `"budget_topup"` (Site 3 — fired separately when young-gen surplus is converted to old-gen budget), `"threshold_adjusted"` (Site 4) | No | Identifies which phase produced this event; use as a filter key when querying. `"evacuation_outcome"` events carry the per-cycle reclaim result (`includedRegions`, `evacuatedBytes`, `reclaimedBytes`, `outcome`, `evacuationBudgetBytes`); `"preparation"` events carry the garbage survey (`collectableGarbageBytes`, `collectableGarbageRegions`, `immediateGarbageBytes`, `immediateGarbageRegions`, `defragRegions`, `unselectedRegions`). **For `"budget_topup"`**: only `budgetTopupRegions` is populated — all outcome and preparation fields are null; this event fires as a distinct notification that the old-gen evacuation budget was augmented from a young-gen surplus, not as an evacuation outcome. Note: in the recommended three-event form, this `phase` field does not exist — each event type implicitly identifies its phase. |
...
1. **Resolved: file as three separate events**. The three phases (`prepare_for_old_collections`, `finalize_mixed_evacs`, `adjust_old_garbage_threshold`) have different cardinalities — Site 1 fires once per old-gen marking cycle; Site 2/3 fire once per mixed evacuation cycle (more frequent). A single event type with a `phase` field conflates two different granularities and produces unavoidable nullable fields. **Decision**: file as three separate events — `jdk.ShenandoahOldGarbageSurvey` (Site 1), `jdk.ShenandoahOldEvacuationOutcome` (Sites 2 and 3), `jdk.ShenandoahOldThresholdAdjust` (Site 4). Each event is independently queryable with no nulls. The `phase` discriminator design remains as a fallback if upstream prefers consolidation.
...
| `gcId` | `GCId::current()` — the GC ID of the concurrent mark cycle being interrupted. Valid at Site 1 (`concurrent_cycle_end()`, called directly from `G1ConcurrentMarkThread::run_service()` where `GCIdMark` is established). At Site 2 (`G1CMTask::handle_abort()`, called from a concurrent worker task thread), `GCId::current()` relies on the thread-local GCId being inherited from the spawning context — this must be verified during implementation (see OQ1). | No (verify GCId availability at Site 2 — see OQ1) | Join key for correlating with `jdk.OldGarbageCollection` (same `gcId`), `jdk.G1BasicIHOP`/`jdk.G1AdaptiveIHOP` (the IHOP threshold that triggered this cycle), and `jdk.G1HeapSummary` (heap state when the mark was interrupted). |
...
[`g1ConcurrentMark.cpp`](https://github.com/openjdk/jdk/blob/master/src/hotspot/share/gc/g1/g1ConcurrentMark.cpp) — `concurrent_cycle_end()` at line ~1282 (abort site) and `G1CMTask::handle_abort()` at line ~2790 (overflow-reset site).
...
3. **Resolved: drop `markCycleCompleted` field**. The field would always be `false` at both emission sites: at the abort site because `has_aborted()` gating the log line implies the cycle did not complete; at the overflow-reset site because `mark_cycle_completed` is not even in scope. A field that is always the same constant carries zero information — same reasoning that dropped `consecutiveViolations` from proposal #2. The `type` discriminator already conveys the completion status: `"abort"` means incomplete; `"overflow_reset"` means the cycle is restarting, not completing.
...
4. **Resolved: drop `gcCause` field**. `gc_cause()` is always `GCCause::_g1_compaction_pause` at all three emission sites — the `GCCauseSetter` is constructed at each call site with this exact value. A field that is a compile-time constant carries no information — same reasoning that dropped `consecutiveViolations` from proposal #2 and `markCycleCompleted` from proposal #17. `escalationPath` is the actual differentiator between the three paths. The `jdk.GarbageCollection` event already records `cause` for consumers who filter by GC cause.
