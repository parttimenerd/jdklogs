...
**Last updated**: 2026-08-18 (pass 44)
...
| `cause` | `shenandoah_concurrent_gc` / `metadata_GC_threshold` / `alloc_failure` / etc. from `GCCause` enum | No | **Urgency indicator**: `shenandoah_concurrent_gc` = normal heuristic-driven cycle (healthy). `alloc_failure` = urgent: an allocating thread stalled waiting for GC to free space; if frequent, heap is undersized for this allocation rate — increase `-Xmx`. `metadata_GC_threshold` = metaspace pressure, not heap exhaustion — typically indicates high-rate class loading, OSGi hot-swap, or reflection-heavy code generating proxy classes. |
...
| `minorFreeBytes` | Available young-gen free bytes from alloc-rate/high-usage rule evaluation; populated when either `rule_minor_allocation_rate_dynamic` or `rule_minor_high_usage` is the evaluated rule | Yes | Young-gen remaining headroom at tick time. `timeUntilMinorOOM` is computed as `minorFreeBytes / maxAllocRate` — so a low `minorFreeBytes` directly drives a low `timeUntilMinorOOM`. If `minorFreeBytes` is declining across ticks while `triggeredMinorRule` is still null, allocation rate is building pressure but has not yet crossed the GC trigger threshold. A sustained decline of > 10% per tick suggests GC will trigger within the next few ticks. Null when neither `rule_minor_allocation_rate_dynamic` nor `rule_minor_high_usage` was evaluated (major rule fired first). |
| `majorFreePercent` | Old-gen free fraction (0–100%) from `rule_major_high_usage` or `rule_major_warmup` evaluation; null when neither rule is the evaluated path | Yes | Old-gen remaining headroom as a percentage of capacity. `rule_major_high_usage` triggers below ~10% (internal threshold; not a user-facing flag). **Interpretation**: > 40% = healthy; 20–40% = filling; 10–20% = near trigger threshold; < 10% = major GC should be running or imminent. **Declining trend**: `majorFreePercent` dropping > 3% per tick means promotions from young gen are outpacing old-GC reclamation — investigate whether tenuring threshold is too low (causing premature promotions) or old GC frequency is too low. **Combined pressure**: if both `majorFreePercent` and `minorFreeBytes` decline simultaneously, the entire heap is under allocation pressure; increasing `-Xmx` is the primary remedy. |
