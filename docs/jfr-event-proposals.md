...
**Last updated**: 2026-08-18 (pass 55)
...
|---|---|---|
| `"Computed"` (stable threshold) | Algorithm converged; the life-decay factor is consistently selecting the same threshold — workload has predictable generational behavior | No action; healthy. Record the stable value as a baseline for comparison when regressions appear. |
...
| `"ZTenuringThreshold"` | Admin override (`-XX:ZTenuringThreshold`) is active; dynamic algorithm result was overridden by the configured value | Verify the static value is still appropriate for current workload characteristics. If old-gen occupancy is growing, the override may be too conservative (too high). Remove the flag entirely to re-enable dynamic adaptation — ZGC's computed value is usually better calibrated than a static choice. |
