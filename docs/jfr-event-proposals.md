...
**Last updated**: 2026-08-18 (pass 47)
...
**Key patterns**:
- `gcuPercent` trending upward across consecutive young GCs → GC CPU overhead is growing; likely means allocation rate is increasing faster than GC throughput. **Actions**: (a) increase `-Xmx` to provide more headroom between collections; (b) check `jdk.ShenandoahCollectionDecision` `triggerType` to confirm the allocation trigger (`rate_average` = steady growth, `rate_accelerated` = phase-change burst); (c) if `muPercent` is also high (> 70%) and trending upward together with `gcuPercent`, the workload is genuinely memory-intensive and heap sizing is the primary lever; if `muPercent` is flat while `gcuPercent` rises, GC is becoming less efficient per allocated byte, which may indicate fragmentation — check `triggerType=fragmentation` events.
