...
**Last updated**: 2026-08-18 (pass 58)
...
- `triggerType=fragmentation` with growing `fragmentationDensityPct` → old gen is becoming sparser; lower `ShenandoahOldGarbageThreshold` to reclaim fragmented regions more eagerly. If `fragmentationDensityPct` is declining across subsequent old-GC cycles, old-gen compaction is working. If it stays low (< 50%) despite repeated old GCs, fragmentation is structural — objects are scattered across many regions and are keeping them alive; consider whether class unloading or reference clearing could help.
