...
**Last updated**: 2026-08-18 (pass 43)
...
|---|---|---|---|
| `startTime` | Standard JFR | No | Verify trim fires at expected interval (`TrimNativeHeapInterval`) |
| `duration` | Standard JFR | No | Trim wall time; > 100ms suggests heavy arena fragmentation or slow OS munmap |
...
| `muPercent` | Mutator utilization 0–100+ — `_most_recent_mu * 100` from `update_utilization()`, independently measured from GC thread CPU time; **not** simply `100 - gcuPercent`. Both fractions are measured against `_active_processors × gc_cycle_period`. On a 16-core machine running at 50% load, both GCU and MU can be far below 100% (remaining = I/O, sleep, other processes). MU can briefly exceed 100% when the CPU-time accounting window boundary falls mid-cycle and threads spike at the boundary — this is a measurement artifact, not a real > 100% utilization; treat MU > 100% as approximately 100% in practice. | No | If `muPercent + gcuPercent << 100`, remaining CPU capacity is idle (I/O wait, blocked threads, other processes on the host). If `muPercent < 50%` and `gcuPercent < 10%`, the application is not CPU-bound — heap sizing and thread counts are not the bottleneck. |
...
| Field | Source | Nullable? | Tuning use |
|---|---|---|---|
| `marginOfError` | `_margin_of_error_sd` from `ShenandoahAdaptiveHeuristics` | Yes | Confidence buffer in GC duration prediction; higher values = heuristic is less certain, so it triggers earlier. Increases after back-to-back degenerated GCs; decreases after smooth concurrent cycles. |
| `triggerType` | `"rate_average"` / `"rate_momentary"` / `"rate_accelerated"` / `"fragmentation"` / `"growth"` / `"expansion_failure"` / `"min_free"` / `"learning"` / `"pending"` / `"other"` | Yes | **Core diagnostic**: which heuristic condition fired |
| `anticipatedGcDurationMs` | ms; rate triggers only — `rate.duration_seconds() * 1000` | Yes | How long the heuristic predicted this GC will take. If increasing over time, GC is getting slower; combined with shrinking headroom, allocation stalls are approaching. |
| `baselineConsumptionBytes` | `rate.baseline_consumption()` = baseline_rate × anticipated duration; rate_average trigger only | Yes | Expected bytes consumed at average allocation rate during the GC — the "budget" the heuristic is protecting against |
| `fragmentationDensityPct` | Density = `used / used_regions_size`; fragmentation trigger only | Yes | Low density (< 50%) = live objects spread sparsely across many regions; increasing density over cycles = old gen is defragmenting via reclamation |
| `fragmentedFreeBytes` | `used_regions_size - used` — fragmentation trigger only | Yes | How many bytes are "free" but trapped inside sparsely populated regions |
| `liveAtPrevMarkBytes` | Live bytes at end of last old marking; growth trigger only | Yes | Baseline for growth calculation: `(currentUsageBytes - liveAtPrevMarkBytes) / liveAtPrevMarkBytes` = growth rate since last old marking |
...
| Field | Source | Nullable? | Tuning use |
|---|---|---|---|
| `immediateGarbage` | Immediate garbage bytes from `prepare_for_old_collections()` | Yes | Regions with 100% garbage; collected free without evacuation cost |
| `immediateRegions` | Immediate garbage region count | Yes | High count = efficient old-gen reclaim; low count = live objects are spread across most regions |
| `mixedCandidates` | Old region mixed collection candidates | Yes | How many old-gen regions the policy identified as collection-worthy |
| `mixedRegionsSelected` | Old regions selected | Yes | Compare to `mixedCandidates` — if selected << candidates, budget or region limits are binding |
| `oldEvacuationBudget` | Old gen evacuation budget in bytes | Yes | How much space is reserved for old-gen object copies during mixed GC |
| `defragRegions` | Defragmentation region count | Yes | Regions selected specifically to reduce old-gen fragmentation (low garbage density but high fragmentation cost) |
...
| Field | Source | Nullable? | Tuning use |
|---|---|---|---|
| `startTime`, `freePassed`, `goodProgress` | Always present | No | Core outcome fields |
| `freeActual`, `freeNeeded` | Always present | No | `freeActual < freeNeeded` = hard gate failed; `freeNeeded = softMaxCapacity * ShenandoahCriticalFreeThreshold / 100` |
| `usedFreed`, `usedNeededToFree`, `usedPassed` | Null if free check failed first | Yes | Bytes reclaimed vs. 1-region threshold; low `usedFreed` = live set is dense, GC found little to collect |
| `internalFragDeltaPct`, `internalFragThresholdPct`, `internalFragPassed` | Null if earlier dimension failed | Yes | Delta in internal fragmentation (holes inside regions); threshold = 1%. If consistently not passing, regions are fragmented but live objects keep them pinned. |
| `externalFragDeltaPct`, `externalFragThresholdPct`, `externalFragPassed` | Null if earlier dimension failed | Yes | Delta in external fragmentation (ratio of small free chunks to total free); threshold = 1%. |
...
|---|---|---|
| `tenuringThreshold` stable at a mid-range value | Algorithm has converged; steady-state workload | No action; healthy |
| `tenuringThreshold` oscillates between `minTenuringAge` and `maxTenuringAge` | Bursty allocation pattern — object lifetime varies cycle to cycle | Check for phase changes in application (e.g., batch bursts). Consider `ShenandoahGenerationalTenuringMortalityRateThreshold` |
| `tenuringThreshold == minTenuringAge` every cycle | Clamp is binding; algorithm wants even more aggressive promotion | Lower `ShenandoahGenerationalMinTenuringAge` if old gen pressure is acceptable |
| `tenuringThreshold == maxTenuringAge` every cycle | Algorithm found no high-mortality cohort; all ages have good survival rates | Objects are genuinely long-lived. If old gen grows, raise `ShenandoahGenerationalMaxTenuringAge` to keep them in young longer, or verify the workload does not have a promotion leak |
...
|---|---|---|
| `"Computed"` (stable threshold) | Algorithm converged; workload has consistent generational behavior | No action; healthy |
| `"Computed"` (oscillating threshold) | Object lifetime distribution varies cycle to cycle | Check for allocation bursts or phase changes in the application |
| `"Computed"` (threshold consistently at 1) | Strong allocation pressure: high `allocatedGarbageRatio` is forcing a low log base, minimizing the threshold | Increase young-gen size if possible; `-XX:ZYoungGenerationSizePercent` |
| `"ZTenuringThreshold"` | Admin override is active; dynamic algorithm disabled | Verify the static value is still appropriate for current workload. Remove the flag to re-enable dynamic adaptation. |
| `"Promote All"` | Emergency: ZGC flushed all young objects to old gen due to allocation pressure | Heap is undersized for this workload. Increase `-Xmx` or verify `SoftMaxHeapSize`. Expect old-gen growth spike. |
...
| `registeredNMethods` | `_nregistered` — current live count from `ZNMethodTable` | No | Size of the nmethod table ZGC must scan each cycle. High values (> 50K) increase per-collection scan overhead. Correlate with GC pause length. |
| `staleNMethodSlots` | `_nunregistered` — zombie entries pending table rebuild; NOT cumulative; resets to 0 after each rebuild | No | **Staleness indicator**: high ratio of `staleNMethodSlots / registeredNMethods` (> 10%) means many table slots hold dead references that still consume scan time. A rebuild clears these — if stale count never decreases, the rebuild trigger is not firing. |
...
| `cardsNotParsable` | `stats->cards_not_parsable()` — cards that returned `G1RemSet::CouldNotParse` and were **re-dirtied for retry** (source: `g1ConcurrentRefineSweepTask.cpp:75-79`): the card region was in an unparsable state so the thread put the card back as dirty for next sweep. | No | **Retry-storm indicator**: sustained non-zero across sweeps means the refinement thread keeps encountering unparsable regions — typically when humongous objects are being allocated concurrently. Consider for the initial proposal: non-zero is rare but diagnostic when it occurs. |
...
| `cardsStillRefersToCset` | `stats->cards_already_refer_to_cset()` — cards found to be already marked as to-CSet (`G1RemSet::AlreadyToCSet` result, `g1ConcurrentRefineSweepTask.cpp:66-69`): these were previously discovered as pointing into the CSet and are being re-encountered. High count = many cards are cycling back through the refinement queue with references that still point into the collection set. | No | Non-zero and growing → a significant number of references are persisting toward CSet regions between sweeps; may increase `Update Remembered Set` work during the next GC pause |
...
3. **Recommendation: link to refinement redesign RFE** — the JFR RFE should reference JDK-8382089/JDK-8383794 as the context for the redesign. The sweep state machine changes in those RFEs reduced observability by removing the old per-thread card-count logs. This event restores structured observability in JFR form.
...
| `threadsWanted` | `new_wanted` from `adjust_threads_wanted()` — [`g1ConcurrentRefine.cpp:610`](https://github.com/openjdk/jdk/blob/master/src/hotspot/share/gc/g1/g1ConcurrentRefine.cpp#L610). Available on the periodic path only (OQ3 recommends periodic-path-only emission). | No | **Primary output**: how many refinement threads the policy currently wants. If consistently at max (`G1ConcRefinementThreads`), refinement is capacity-constrained — increase `-XX:G1ConcRefinementThreads` |
| `pendingCards` | `policy->current_pending_cards()` — current total pending dirty cards from the card queue | No | Current backlog; if growing between policy ticks, refinement is falling behind. Compare to `pendingCardsTarget`: sustained `pendingCards > pendingCardsTarget` means the policy has set a target it cannot achieve with current thread count |
| `pendingCardsFromGC` | `pending_cards_from_gc()` — cards dirtied by GC-internal operations (remembered-set rebuilding, evacuation); available on periodic path | No | Distinguishes GC-generated card traffic from mutator write traffic; `pendingCards - pendingCardsFromGC` = mutator-driven backlog. High `pendingCardsFromGC` = GC itself is contributing significantly to its own refinement backlog |
...
| `predictedPendingCards` | `_threads_needed.predicted_cards_at_next_gc()` — projected backlog at the next GC pause: `num_cards + incoming_rate × predicted_time_until_next_gc`. Source: [`g1ConcurrentRefineThreadsNeeded.cpp:64`](https://github.com/openjdk/jdk/blob/master/src/hotspot/share/gc/g1/g1ConcurrentRefineThreadsNeeded.cpp#L64) | No | **Keep — resolved (OQ2).** The projected backlog at the next GC: if `predictedPendingCards > pendingCardsTarget` even when `threadsWanted` is at max, the refinement system cannot keep up regardless of thread count — the only fix is workload reduction or write-barrier profile change |
| `predictedRefineRate` | `analytics->predict_concurrent_refine_rate_ms()` — predicted cards/ms that the refinement threads can process, derived from historical sweep throughput | No | **Capacity side**: `predictedRefineRate × threadsWanted × timeUntilNextGC` = cards refinement expects to clear before next pause. If this is less than `pendingCards`, the backlog will spill into the GC pause |
| `dirtiedCardRate` | `analytics->predict_dirtied_cards_rate_ms()` — predicted cards/ms being dirtied by mutator writes, derived from recent write activity | No | **Demand side**: if `dirtiedCardRate > predictedRefineRate × threadsWanted`, the policy will fall behind and the backlog will grow. The policy uses this ratio to determine `threadsWanted` |
| `goalMs` | `_pending_cards_target` time equivalent: the time window within which refinement is expected to clear its backlog, derived from `_mmu_tracker->max_gc_time() × G1RSetUpdatingPauseTimePercent/100`. On the periodic path (recommended, see OQ3): the time window the policy uses to decide how many threads are needed. | No | **Reference target**: compare `pendingCards × (1/predictedRefineRate) / threadsWanted` against `goalMs` to estimate whether the current thread count can clear the backlog in time |
...
| `timeUntilNextGC` | `_threads_needed.predicted_time_until_next_gc_ms()` — predicted milliseconds until the next GC pause, derived from the GC analytics model | No | Time budget for refinement: `timeUntilNextGC × predictedRefineRate × threadsWanted / 1000` ≈ cards that can be cleared before next pause; compare to `pendingCards` to see if refinement will catch up |
| `exceededGoal` | Boolean: `pendingCards > pendingCardsTarget` — pending backlog exceeds the policy's target | No | `true` repeatedly → refinement cannot meet its target; if `threadsWanted` is already at max, the only resolution is reducing write rate or accepting larger pause-time contribution from card processing |
...
|---|---|---|
| `"Predicted time too high"` | Pause time budget exhausted before `maxRegions`; region was predicted to exceed remaining budget | Increase `-XX:MaxGCPauseMillis` (allows more old-gen work per pause), or reduce old-gen region liveness threshold via `-XX:G1MixedGCLiveThresholdPercent` (avoid dense regions) |
| `"Maximum number of regions reached"` | Hard cap on old-gen regions per pause hit; `availableRegions` still > 0 | Increase `-XX:G1OldCSetRegionThresholdPercent` (default 10% of heap regions) to allow more regions per mixed pause |
| `"Region amount reached min"` | `minRegions` met exactly; time budget was 0 but forced inclusion complete | Check `overBudgetRegions` > 0 — these forced additions will extend pause. Consider decreasing `-XX:G1MixedGCCountTarget` to spread work over more pauses |
| `"Marking candidates exhausted"` | All marked old-gen candidates were consumed | Healthy state if occupancy trend is flat. If old gen still grows, marking is not completing fast enough — check IHOP threshold via `jdk.G1AdaptiveIHOP` |
| `"Retained candidates exhausted"` | All retained (deferred optional) candidates consumed | Retained list is keeping up; no backlog |

**Additional patterns**:
- `overBudgetRegions > 0` frequently → the GC is forced to add over-budget regions to meet its minimum; pause times will exceed predictions. This often indicates old-gen backlog is accumulating — consider more frequent mixed GC via `-XX:G1MixedGCCountTarget`.
...
- `selectedRegions / availableRegions` ratio < 0.3 consistently → less than 30% of candidates are being collected per pass; old-gen reclamation is slow relative to its accumulation rate. Increase `MaxGCPauseMillis` or reduce `G1MixedGCLiveThresholdPercent`.
...
| `timeUntilMinorOOM` | From alloc-rate rule (`rule_minor_allocation_rate_dynamic`): `time_until_oom` computed from allocation rate model | Yes | Seconds until OOM at current allocation rate; null if the dynamic alloc-rate rule was not the firing rule (e.g., a major rule preempted minor evaluation, or a different minor rule fired instead). Low value = imminent allocation failure |
| `minorFreeBytes` | Available young-gen free bytes from alloc-rate/high-usage rule evaluation; populated when either `rule_minor_allocation_rate_dynamic` or `rule_minor_high_usage` is the evaluated rule | Yes | Young-gen remaining headroom at tick time. `timeUntilMinorOOM` is computed as `minorFreeBytes / maxAllocRate` — so a low `minorFreeBytes` directly drives a low `timeUntilMinorOOM`. If `minorFreeBytes` is declining across ticks while `triggeredMinorRule` is still null, allocation rate is building pressure but has not yet crossed the GC trigger threshold. |
| `majorFreePercent` | Old-gen free fraction (0–100%) from `rule_major_high_usage` or `rule_major_warmup` evaluation; null when neither rule is the evaluated path | Yes | Old-gen remaining headroom as a percentage of capacity. `rule_major_high_usage` triggers when this falls below a threshold (~10-20%); declining `majorFreePercent` across ticks with no major GC triggered means promotion from young gen is filling old gen faster than old GC is running. If both `majorFreePercent` and `minorFreeBytes` are declining, heap is under combined pressure — consider increasing `-Xmx` or reducing live set. |
...
2. **Recommendation**: gate the event on "at least one rule fired OR `timeUntilMinorOOM < threshold`". Emitting every second (~86,400/day) even when idle generates significant recording overhead with no diagnostic value. The recommended filter: emit when `triggeredMinorRule != null || triggeredMajorRule != null || (timeUntilMinorOOM != null && timeUntilMinorOOM < 30.0)`. This captures all trigger decisions AND early-warning ticks where the heap is approaching exhaustion even though GC hasn't started yet — the precise gap that ZGC's proactive model is designed to detect.
3. **`timeUntilMinorOOM` struct accumulation — concrete design**: `timeUntilMinorOOM` is computed as a local variable inside `rule_minor_allocation_rate_dynamic()` and does not flow up to `start_gc()`. The implementation requires a small output struct:
...
`make_minor_gc_decision()` returns `ZDirectorResult` alongside the `GCCause::Cause`. `rule_minor_allocation_rate_dynamic()` populates `time_until_oom` into the struct before returning. `minorFreeBytes` can alternatively be derived from `stats._heap` fields already available in `start_gc()`. The struct change is ~15 lines across 3 functions — non-trivial but well-bounded. This is the correct implementation approach; the alternative (emitting without `timeUntilMinorOOM`) loses the most diagnostic value of the event.
...
| `pauseGoalMs` | `_gc_pause_goal_sec * 1000` from `MaxGCPauseMillis` (default `max_uintx-1` ≈ unlimited) | No | Active only when `-XX:MaxGCPauseMillis=N` is set. When set, pause control takes priority over throughput goal (pause > throughput in the oracle priority order). |
| `gcDistanceSec` | `_gc_distance_seconds_seq.davg()` — exponentially-weighted average of inter-GC wall-clock intervals | No | **GC frequency indicator**: how far apart young GCs are, on average. Short distance (e.g. < 0.3s) = very frequent GC; heap may be too small or allocation rate too high. Policy uses this to keep GC neither too rare nor too frequent. |
| `gcDistanceSecLast` | `_gc_distance_seconds_seq.last()` — raw last inter-GC interval (no smoothing) | No | Spike detection: sudden drop vs `gcDistanceSec` means a single very fast GC. Sustained difference means workload changed. |
...
| `edenSizingBranch` | Which branch of `compute_desired_eden_size()` fired: `"throughput_grow"` (Branch 1: `mutator_time_percent() < throughput_goal`, eden grows), `"pause_shrink"` (Branch 2: `minor_gc_time_estimate() > gc_pause_goal_sec()`, eden shrinks), `"distance_grow"` (Branch 3: GC too frequent vs. distance goal, eden grows), `"distance_shrink"` (Branch 4: GC too rare vs. distance threshold, eden shrinks slightly), `"unchanged"` (no condition triggered). Replaces `throughputEdenIncrease` — non-null, carries the branch label, more readable than the raw delta. | No | Which constraint drove the eden sizing this cycle. **`throughput_grow` followed by `pause_shrink` on the next cycle** = policy oscillation: throughput goal and pause goal are pulling in opposite directions — raise `-XX:MaxGCPauseMillis` or unset it. `"unchanged"` majority = policy has converged. |
...
| `shrinkBytes` | Old gen shrink amount this cycle; 0 if no shrink triggered | No | Non-zero → policy is actively shrinking old gen. **Risk**: if `promotedBytesEstimate` spikes in the next few cycles after a shrink, the now-smaller old gen may overflow. Watch for `shrinkBytes > 0` followed by `survivorOverflow=true` or a `promotedBytesEstimate` jump — that sequence indicates the shrink was too aggressive. |
