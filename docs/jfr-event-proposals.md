...
**Last updated**: 2026-08-18 (pass 40)
...
| `pendingCardsTimeMs` | GC-pause: actual time to process pending cards in the last GC pause; periodic path: not available. **RECOMMENDED TO DROP** — if periodic-path-only emission is adopted (see open question 3), this field is never available. The GC pause card-processing time is separately visible by correlating `jdk.GarbageCollection` `Update Remembered Set` phase duration. | Yes (null on periodic path) | Compares against `goalMs` — the GC-pause dimension of whether card processing met its goal |
