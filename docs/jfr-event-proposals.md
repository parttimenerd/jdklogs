...
**Last updated**: 2026-08-19 (pass 78)
...
| `phase` | String discriminator: `"preparation"` (Site 1), `"evacuation_outcome"` (Site 2 — normal, all-pinned, or no-budget paths), `"budget_topup"` (Site 3 — fired separately when young-gen surplus is converted to old-gen budget), `"threshold_adjusted"` (Site 4) | No | Identifies which phase produced this event; use as a filter key when querying. `"evacuation_outcome"` events carry the per-cycle reclaim result; `"preparation"` events carry the garbage survey. Note: in the recommended three-event form, this `phase` field does not exist — each event type implicitly identifies its phase. |
